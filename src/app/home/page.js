...
export default function HomePage() {
  ...
  const fetchTransactionsCallback = useCallback(
    async (userId) => {
      ...
      // Your existing fetchTransactions code
      // Replace any references to getCategoryIcon with this inner function
      try {
        const transactionsRef = collection(db, "transactions");
    ...
    try {
      ...
      if (newTransaction.image) {
        ...
        if (!imageUrl && newTransaction.image) {
          setTransactionError(
            "Failed to upload receipt image. Transaction was not saved."
          );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-yellow-50 via-orange-50 to-red-50">
      ...
      <div
        className={`fixed top-0 left-0 h-full bg-white shadow-xl z-40 transform transition-transform duration-300 ease-in-out w-64 lg:translate-x-0 ${
        ...
        <nav className="bg-white shadow-sm">
          <div className="px-4 py-4 flex items-center justify-between">
            <div className="flex items-center">
              ...
              <h1 className="ml-2 text-xl font-bold text-gray-800 lg:hidden">
                Treasure Tracker
              </h1>
            </div>
                                  ...
                                  <div className="w-10 h-10 bg-gray-100 rounded-lg overflow-hidden relative">
                                    <Image
                                      ...
                                      onError={(e) => {
                                        ...
                                        e.target.src =
                                          "https://placehold.co/40x40/gray/white?text=No+Image";
                                      }}
                                    />
                                  </div>
                                ) : (
